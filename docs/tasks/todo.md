...
- [x] 删除前核对 7 个精确目标均位于 `src/assets/art/sketchbook/` 且未被运行时代码引用
- [x] 删除 7 张白边废稿，不触碰其他素材
- [x] 同步废稿清理记录，复核正式六图一致性、剩余文件和 Git 差异

## 计划确认

- 删除范围：`about / book-master / cover / gallery / guestbook / journal / projects` 的 `*-white-edge-backup.png`，预计释放约 `13.32 MiB`。
- 保留范围：六张 `*-unified.png`、`book-master-unified.png`、六张旧稳定名素材及五张 `*-landscape-backup.png`。
- 验证：确认目标无运行时引用，运行册页素材一致性检查、格式检查与 `git diff --check`。

## 结果审查

- 已删除 `about / book-master / cover / gallery / guestbook / journal / projects` 共 7 张 `*-white-edge-backup.png`，释放约 `13.32 MiB`；这些文件原为未跟踪的瑕疵废稿，Git 无法直接恢复。
- 当前目录剩余 18 张图片：在用六图、`book-master-unified.png`、六张旧稳定名素材及五张山水备用图均完整保留。
- 正式六图一致性检查通过：均为 `1760×1240` RGBA，边界 `(89,270)–(1671,966)`，`brightEdgeRatio=0`；格式检查与 `git diff --check` 通过。

# 当前任务：对齐上游素材与翻页流畅度

目标：以 `D:\MyBlog\sketchbook\sketchbook` 的真实图片和 `index.html` 为基准，建立可重复的尺寸 / 几何 / 材质诊断，重新生成六张同构东方水墨册页，并只修复经差分证明确实影响翻页观感的实现偏差；不复用既有站点画面内容，不删除任何备用素材，不新增依赖，不提交、不推送。

- [x] 清点上游与本站全部翻页素材，记录像素、比例、格式、通道、体积、边缘、书脊、纸纹和构图统计
- [x] 建立可自动运行的素材一致性检查，先证明当前六图在哪些指标上不一致
- [x] 对照上游 `index.html` 与本站 `Hero.astro` / `hero-motion.ts` / CSS，按渲染、几何、动画、加载和合成层分类差异
- [x] 先确定一张统一母版，再以其为风格参考逐张生成六种水墨题材，所有最终图采用完全相同的画布和书本几何
- [x] 保留当前六张与既有山水备用图，使用新的稳定文件名接入，不覆盖或删除历史素材
- [x] 在黑底定位并清除统一母版四周的矩形白边；首版曾以 `*-white-edge-backup.png` 暂存，后按用户确认清理
- [x] 让一致性检查转绿，并在原仓库与本站真实浏览器中对比稳定页、拖拽与自动翻页中间帧
- [x] 完成格式、类型、构建、E2E、布局、控制台、性能和差异检查

## 计划确认

- 诊断优先：在看完上游真实图片和运行效果前不生成、不改翻页参数；先用同一脚本给两边图片建立可比较数据，再列出 3–5 个可证伪原因并逐项验证。
- 素材母版：从上游图片提取画布比例、书脊位置、页角半径、叠页厚度、纸纹频率、亮度与边缘颜色；本站题材保持东方水墨，但六张只改变画面主体，不改变纸张、书形、照明、视点或留白结构。
- 生成路径：使用内置 `image_gen` 的全新生成模式；上游图片和统一母版仅作为明确标注的样式 / 几何参考，不作为编辑目标，不复制其建筑或现有站点画面。每张生成后统一无损裁切 / 缩放到诊断得出的单一尺寸，并读取真实像素验证。
- 代码边界：优先保留上游 `SPAN / BETA / 18 段曲面 / 阈值 / 弹簧 / 阴影`；只有差分和浏览器测量证明某项本站适配影响手感时才做最小修改，内容 DOM、路由、Header 与首页后续章节不在本轮范围。
- 备份策略：当前 `cover.png` 与另外五张新图全部改名保留；既有 `*-landscape-backup.png` 继续保留。新一轮素材使用独立版本名，验收通过后再切换 import。
- 验证：使用自动一致性检查作为红 / 绿反馈环，再以原仓库和本站的真实浏览器中间帧、长拖 / 短拖 / 循环、1707×735、1440px、390×844、320px、明暗主题和控制台作为终验。

## 结果审查

- 六张正式素材均为 `1760×1240` RGBA，书本可见边界统一为 `(89,270)–(1671,966)`；以同一书体母版承载山水、文房、纸模型、相纸、茶盏梅枝和花鸟六种题材。自动检查确认六图边界偏差为 0，四周高亮边缘比例为 0；既有素材与山水备用图均未删除，7 张白边废稿后按用户确认清理。
- 白边根因不是 CSS 外框，而是生成母版在透明轮廓内残留了近白 RGB 边缘；现以真正透明画布重建轮廓，并给可见纸沿统一保留约 1px 的暖灰纤维收边。真实浏览器覆盖 `1707×735` 明暗主题、翻页稳定页 / 中间帧及 `390×844` 暗色移动端，未再出现矩形白框，移动端 `scrollWidth` 等于 `clientWidth`，控制台 0 errors / 0 warnings。
- 对照上游后只迁移确有影响的画布与加载差异：Hero 恢复 `1760 / 1240` 画布比例，可见纸带遮罩统一为 `21.8%`，去除本地额外的矩形裁切 / 结构阴影，并在开场前用上游式 `Image.decode()` 预解码。18 段曲面、`SPAN / BETA`、提交阈值、两组弹簧、阴影算法与书本显示尺寸均未修改。
- 测量同一箭头翻页时，上游 / 本站分别采样 57 / 60 帧；本站平均帧间隔约 `23.61ms`、最慢帧约 `49.9ms`，未慢于上游的 `24.85ms / 50.1ms`。因此未为追求指标继续改动 DOM 覆盖、本站开场缓动或 3px 拖拽门槛，避免效果漂移。
- 新文件名暴露了 E2E 延迟路由只匹配旧素材名的问题；测试缝已兼容 `*-unified`，预载期箭头接管重新验证通过。最终门禁：`pnpm format:check`；`pnpm check` 为 58 文件 0 问题；`pnpm build` 生成 24 页、96 个优化图像和 3 页 Pagefind 索引；`pnpm test` 为 11/11；Impeccable layout 扫描返回 `[]`；`git diff --check` 通过。
- 本轮未新增依赖、未提交、未推送。代码图覆盖未记录缺口但相关文件均为 `metadata_changed`，结论已以直接源码差异、像素检查与真实浏览器结果复核。

# 当前任务：Hero 册页题材多样化

目标：保留卷首山水和现有翻页效果，从零生成笺录、器作、光影、此间、留墨五张新册页；旧山水素材改名留作备用，不复用其画面内容，不修改 Hero 交互、尺寸或翻页内核，不新增依赖，不提交、不推送。

- [x] 将五张现有山水素材改名为 `*-landscape-backup.png`，确认卷首与备用图完整保留
- [x] 使用内置图像生成逐张从零生成五种题材，不把现有素材作为编辑目标或引用输入
- [x] 校验新图的纸本质感、中央书脊、叠页、留白、尺寸、色彩与无文字要求
- [x] 接入稳定文件名并同步此间 / 留墨替代文本、设计契约与进度
- [x] 在桌面、移动端、明暗主题、稳定页与翻页中间帧验证质感和文字可读性
- [x] 完成格式、类型、构建、E2E、布局与差异检查

## 计划确认

- 素材范围：`cover.png` 保持当前山水；其余五张全部重新生成，题材依次为砚台纸笺、纸模型、相纸、茶盏花枝、花鸟留言笺。
- 生成方式：使用内置 `image_gen` 的全新生成模式，每种题材独立调用；不输入或编辑现有山水素材。旧图仅在项目内改名备份，Git 可继续追踪。
- 视觉约束：新图使用相同的暖白宣纸、可见纸纤维、左侧叠页、中央书脊、细钢笔式墨线、透明水彩与暖赭灰墨；主体落在下方约 45%，不生成文字、日期、印章字样、水印、外框或硬封底。
- 受保护范围：不修改 `hero-motion.ts`、Hero CSS、页面尺寸、18 段曲面、弹簧、阴影、真实 DOM 文案、路由或首页后续章节。
- 验证：逐卷检查六张稳定页和翻页中间帧，覆盖 `1707×735`、1440px、390×844、320px、明暗主题、低动态、无 JS、横向溢出与控制台；最终运行项目全部门禁。

## 结果审查

- 笺录、器作、光影、此间与留墨均由内置图像生成从零独立生成，未输入、编辑或引用旧素材；题材依次为砚台纸笺、纸模型与榫卯样件、相纸与窗格投影、茶盏折图与梅枝、花鸟留言笺。卷首山水保持不变。
- 五张旧山水分别保留为 `journal-landscape-backup.png`、`projects-landscape-backup.png`、`gallery-landscape-backup.png`、`about-landscape-backup.png` 与 `guestbook-landscape-backup.png`；逐一核对后均与修改前的 Git blob 完全一致。新图沿用原稳定文件名，保持 `1852–1855×848–849`、RGB / sRGB、无透明通道和同构册页几何。
- 代码仅同步 `Hero.astro` 的此间 / 留墨 / 光影替代文本与设计契约；未修改 `hero-motion.ts`、Hero CSS、18 段曲面、弹簧、阴影、尺寸、路由、首页后续章节或依赖。
- 项目门禁通过：`pnpm format:check`；`pnpm check` 为 57 文件 0 问题；`pnpm build` 生成 24 页、96 个优化图像和 3 页 Pagefind 索引；`pnpm test` 为 11/11；Impeccable 全站 layout 扫描返回 `[]`。
- 内置浏览器逐卷检查 `1707×735` 浅色六张稳定页、80ms 曲面中间帧、`1440×900` 浅色、`390×844` 暗色完整循环及移动曲面中间帧、`320×844` 暗色；320px 下书本宽约 272px，稳定后 `scrollWidth` 等于 `clientWidth`，控制台 0 errors / 0 warnings。
- 本轮未提交、未推送；图像生成原始输出继续保留在 Codex 生成目录，项目内旧山水备用图未删除。

# 上一任务：以原仓库内核修复 Hero 开场竞态

目标：机械迁移原仓库的单一动画调度与 `endIntro / startTurn / step` 状态语义，修复预载期和开场翻页期间用户输入与自动翻页竞争；保持几何、阴影、素材、尺寸、正常手感及其他审查项不变，不新增依赖，不提交、不推送。

- [x] 核对上游状态机、本地调用链和索引覆盖，确认根因与最小迁移边界
- [x] 先写公共界面回归，复现预载期方向键打断后开场仍恢复
- [x] 迁移单一 `spring / tick` 调度和原版 `endIntro / startTurn / step` 语义
- [x] 覆盖开场翻动期间的方向键与箭头中断，确认用户翻页后不再续播
- [x] 复跑既有 Hero 交互、会话、移动端、低动态和无 JavaScript 回归
- [x] 同步 `DESIGN.md`、`docs/progress.md` 与本节结果审查
- [x] 完成格式、类型、构建、E2E、差异检查和真实浏览器验证

## 计划确认

- 上游基准：优先机械迁移 `spring / tick`、`startTurn / step` 与 `endIntro / riffleStep`；仅为上游未覆盖的预载窗口增加 `introOn` 取消检查，不引入自定义运行编号或状态管理器。
- 公共测试缝：Playwright 只观察可见卷名、左右箭头可用性及等待后的页面稳定性；以素材请求延迟控制预载时序，不读取或断言客户端私有变量。
- 受保护内核：不修改 18 段曲面、`SPAN=.449`、`BETA=.60`、`.42 / 1.1` 阈值、`170/26` 与 `150/24` 弹簧、`applyTurn()`、CSS、素材和书本尺寸；六卷、会话、移动同构、低动态、无 JS 与站内路由继续按本站契约运行。
- 中断终态：预载或开场期间的方向键 / 左右箭头先终止开场；自动页正在翻动时结算到 `turn.to`，随后执行用户请求，且本会话不恢复开场。拖拽热区在开场期间继续门控。
- 验证：红绿测试后执行 `pnpm format:check`、`pnpm check`、`pnpm build`、`pnpm test`、`git diff --check`，并在真实桌面与 390px 浏览器检查稳定页和翻页中间帧。

## 结果审查

- `hero-motion.ts` 现按上游使用单一 `TurnAnimation` 与 `tickTurn()` 驱动手动弹簧和开场 tween；`startTurn()` 先取消旧动画并结算旧页，`step()` 先结束开场再执行用户翻页，预载完成后也会检查开场是否已被取消。六卷数量、会话策略、移动同构与现有 smoothstep 开场缓动保持本站契约。
- 新增三条公共界面回归：延迟素材时方向键接管、延迟素材时外侧箭头接管、开场翻动中方向键接管。红测首先稳定复现“用户翻页后箭头仍被开场占用”，修复后三项均验证等待超过原开场时长也不再续播。
- 全量回归发现并修复双击复位产生的重复短点按计时：第二次短点按现在先取消旧计时器，避免上游 `step()` 语义把随后显式翻页拉回卷首；既有拖拽、回弹、循环、键盘、缩放、导航、首访 / 回访、移动、低动态和无 JavaScript 测试继续通过。
- 项目门禁通过：`pnpm format:check`；`pnpm check` 为 57 文件 0 问题；`pnpm build` 生成 24 页、96 个优化图像和 3 页 Pagefind 索引；`pnpm test` 为 11/11；`git diff --check` 通过。
- 真实 Chrome 覆盖 `1707×735` 与 `390×844` 的开场中间帧和稳定终态：均为 18 段曲面，书宽分别为 736px / 342px，`scrollWidth` 等于视口宽；开场落回“卷首 01 / 06”，控制台 0 errors / 0 warnings，中间帧无白框、黑缝或曲面压平。浏览器和 preview 已关闭。
- 差异未修改 `SEGMENTS / SPAN / BETA`、提交阈值、两组弹簧、`applyTurn()`、Hero CSS、册页素材或书本尺寸；本轮未提交、未推送。
