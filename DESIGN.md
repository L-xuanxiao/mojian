...
- **暗室底片：** 灯箱每次打开只播放一次 520ms 翻底片，底片框从 `rotate(-1.2deg) scale(.96)` 回正；纸白曝光层最高 0.18、360ms 消退。齿孔与曝光只由必要包装层的伪元素绘制，图片始终 `filter: none`；相邻切换统一 250ms 且不重复曝光，关闭沿用 250ms 共享图像回卷。
