...
主轴是一只 84rem 的展台（`--container-stage`）：页眉、页脚与几乎全部页面区块都落在其上，左右页边距 `1.25rem → 2rem → 2.5rem`（sm/md/lg）。文章与项目正文收窄至 42rem 裸值（约 40 字/行）。普通章节采用紧凑的编辑节奏，通常落在 `py-16 md:py-24`；只有 Hero、桌面长卷和暗室可以占用视口级停顿。大胆来自尺度冲突、跨栏和纸层错位，不来自无内容的长空段。

### 四类卷首

- **Reading / Folio：** 题名居左，纸本文物抬至右上，正文入口保持平静。
- **Exhibition：** 图像跨越展台栅格并突破右缘，题名与展签形成前后景。
- **Darkroom：** 卷首直接落入深墨场，大题与主影像共享暗室，不另铺白纸模板。
- **Personal：** 手记式跨页，题名与案头纸笺错位，允许一处克制的纸张倾斜。

`SectionHeading` 是六章共同锚点：大号书法题名、方印编号、竖排英文侧注与横向墨线必须形成清晰尺度冲突。首页六段各有节拍，但仅桌面长卷承担长时间线；移动端、无 JS 与低动态始终呈现完整静态终态。

### Named Rules

**The 峰值留白 Rule.** 视口级停顿只属于 Hero、桌面长卷与暗室；普通章节优先压缩至内容所需高度。若删去一个空段不损失叙事，就应删去。
...
- 当前页以图标落朱砂、题字轻微定场和 2px 短线标章；hover 才写出 1px 朱砂墨线
...
`data-reveal-variant` 只是一份内部 DOM / CSS 动效契约，不是公开组件 API。可用值为 `intro-reading`、`intro-exhibition`、`intro-darkroom`、`intro-personal`、`folio`、`spread`、`exhibit`、`contact-sheet` 与 `stagger`：分别表达题字遮罩、纸层揭开、章节墨线、跨页错峰、展签错峰、暗室显影与普通列表揭示。CSS 负责静止终态、hover、按压、遮罩与细线；GSAP 只承担 Hero、桌面长卷、章节一次性揭示与有限视差。所有前置隐藏仅在 `html.js`、非低动态条件下成立，异常时必须直接显示完整内容。
