...
访客如入主人的书房：四壁是宣纸，器物皆纸墨，朱砂印泥搁在案头，只在落款时落下。系统只有三种材料——纸（底色）、墨（文字与线条）、朱砂（唯一强调色），层级靠字号、字重、留白与 1px 墨线拉开，拒绝卡片盒与色彩噪音。气质是沉浸流动与精致工艺感的合体：页面过渡如墨晕染（旧页淡去、新页自卷心 `clip-path: circle` 展开），图像如装裱揭晓（深纸遮罩自右揭去），光影章节如入暗室显影——每一处动效都对应一个书房里的动作。
...
- **Do** 让动效只动 `transform`/`opacity`，时长取 150/250/400ms 档，曲线用 `--ease-ink`（印章反馈才用 `--ease-seal`）。
